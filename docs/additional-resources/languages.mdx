...
Our APIs currently support **184 languages** from around the world, including right-to-left (RTL) languages. Each language is identified by a unique ISO 639-1 two-letter code.
...
```javascript
// Example API request translating Engligh to Spanish
const result = await jigsawstack.translate({
  current_language: "en", // Using the language code for English
  target_language: "es", // Using the language code for Spanish
  text: "Hello, world!", 
});
```
...
Browse the complete list of supported languages below:
...
| cr | Cree | Nehiyaw | |
| cs | Czech | Česky | |
| cu | Old Church Slavonic | словѣньскъ / slověnĭskŭ | |
| cv | Chuvash | Чăваш | |
| cy | Welsh | Cymraeg | |
| da | Danish | Dansk | |
| de | German | Deutsch | |
...
The following languages are displayed right-to-left:
