...
<ParamField body="return_type" type="string" default="binary">
  ...
  <ul>
    <li>`binary` - Returns the raw binary data of the generated file</li>
    <li>`url` - Returns a URL to download the generated file</li>
    <li>`base64` - Returns the generated file encoded as a base64 string</li>
  </ul>
</ParamField>
...
The API response varies based on the `return_type` parameter:

- When `return_type` is `"binary"` (default), the API returns the generated file (image or PDF) directly in the response body as binary data.
- When `return_type` is `"url"`, the API returns a JSON object with a `url` field containing a download link for the generated file.
- When `return_type` is `"base64"`, the API returns a JSON object with a `base64` field containing the base64-encoded string of the generated file.
...
4. **Return Type Selection**:
   - Use `return_type: "binary"` for direct file processing or when you need the raw file data
   - Use `return_type: "url"` when you want to provide download links or defer file processing
   - Use `return_type: "base64"` when you need to embed the file data in JSON or transmit it as text
