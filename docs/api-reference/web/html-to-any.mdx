...
<ParamField body="return_type" type="string" default="binary">
  Specifies the format of the API response. Supported values:
  <ul>
    <li>`binary` - Returns the raw binary data of the generated image or PDF</li>
    <li>`base64` - Returns the image or PDF data as a base64-encoded string</li>
    <li>`url` - Returns a URL to download the generated image or PDF</li>
  </ul>
</ParamField>
...
- If `return_type` is "binary" (default), the API returns the generated file (image or PDF) directly in the response body as binary data.
- If `return_type` is "base64", the API returns a JSON object with a `data` field containing the base64-encoded string of the generated file.
- If `return_type` is "url", the API returns a JSON object with a `url` field containing a download link for the generated file.
...
4. **Return Type Selection**:
   - Use `return_type: "binary"` for direct file downloads or when processing the file immediately
   - Use `return_type: "base64"` when you need to embed the image data in JSON responses or for easy client-side handling
   - Use `return_type: "url"` when you want to provide a download link to users or when you need to defer the file retrieval
