...
<Info>
  Speech to Text processing is billed at 1 invocation per 25 seconds of
  processing time. This refers to computational time, not audio length - a
  1-hour audio file might only take 20 seconds to process, costing just 1
  invocation.
</Info>
