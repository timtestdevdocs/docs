...
<ParamField body="url" type="string">
  URL of a PDF document to summarize.
  Not required if `text` or `file_store_key` is provided.
</ParamField>
...
<ParamField body="file_store_key" type="string">
  ...
  Not required if `text` or `url` is provided.
</ParamField>
...
<ParamField body="max_points" type="number" default="2">
  The maximum number of bullet points to generate when `type` is set to `points`.
