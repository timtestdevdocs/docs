---
title: "Passport"
api: "GET https://api.jigsawstack.com/v1/validate/passport"
description: "Validate any passport number."
---

### Query

<ParamField query="country_code" type="string" required>
  The country code of the passport.
</ParamField>

<ParamField query="passport_number" type="string" required>
  The passport number to validate.
</ParamField>

<Snippet file="header.mdx" />

### Response

<ResponseField name="success" type="boolean">
  Indicates whether the call was successful.
</ResponseField>

<ResponseField name="message" type="string">
  The message returned by the API.
</ResponseField>

<ResponseField name="valid" type="boolean">
  Indicates whether the passport number is valid.
</ResponseField>

<RequestExample>

```javascript Node.js
const endpoint = "https://api.jigsawstack.com/v1/validate/passport";
const result = await fetch(endpoint);
const data = await result.json();
```

```bash cURL
curl --location --request GET 'https://api.jigsawstack.com/v1/validate/passport' \
--header 'x-api-key: <your-api-key>'
```

</RequestExample>

<ResponseExample>

```json Response
{
  "success": true,
  "message": "Validated successfully",
  "valid": true
}
```

</ResponseExample>
