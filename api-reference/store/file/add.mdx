...
<Info>
  Uploaded files are private by default. To make them public or accessible,
  attach your `public key` to the `x-api-key` query parameter.
</Info>

```javascript
const publicUrl = `${result.url}?x-api-key=${publicKey}`;
```
